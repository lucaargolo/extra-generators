...
class FluidGeneratorBlock(settings: Settings, generatorConfig: ModConfig.Generator, val fluidFuelMap: (Fluid) -> FluidGeneratorFuel?): AbstractGeneratorBlock(settings, generatorConfig), InventoryProvider {

    override fun getInventory(state: BlockState?, world: WorldAccess?, pos: BlockPos?): SidedInventory {
        return (world?.getBlockEntity(pos) as? FluidGeneratorBlockEntity)?.itemInv ?: SimpleSidedInventory(0, { _, _ -> false }, { _, _ -> false }, { intArrayOf(0, 1) })
    }
    ...
    override fun onUse(state: BlockState, world: World, pos: BlockPos, player: PlayerEntity, hand: Hand, hit: BlockHitResult): ActionResult {
        (world.getBlockEntity(pos) as? FluidGeneratorBlockEntity)?.let{ InventoryUtils.interactPlayerHand(it.fluidInv, player, hand, canExtract = false) }?.let {
    ...
    override fun createBlockEntity(pos: BlockPos, state: BlockState) = FluidGeneratorBlockEntity(pos, state)

    override fun <T : BlockEntity?> getTicker(world: World?, state: BlockState?, type: BlockEntityType<T>?): BlockEntityTicker<T>? {
        return checkType(type, BlockEntityCompendium.FLUID_GENERATOR_TYPE, AbstractGeneratorBlockEntity.Companion::commonTick)
    }
